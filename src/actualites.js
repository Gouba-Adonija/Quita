...
class ActualitesManager {
  ...
  async init() {
    await Promise.all([this.loadActualites(), this.loadPopularContent()]);
    this.checkUrlParams();
  }
        ...
        featuredActu.titre
                    ...
                    <div class="featured-content">
                        ...
                        <div class="featured-meta">
                            ...
                            <a href="#" class="read-more" onclick="actualitesManager.showActualiteDetails(${
                              featuredActu.id
                            })">
                                Lire la suite
                            </a>
  ...
  displayActualites(actualites) {
    this.newsContainer.innerHTML = actualites
      .map(
        (actu) => `
            <div class="news-card" data-category="${actu.categorie}">
                ...
                <div class="news-content">
                    ...
                    <div class="news-meta">
                        ...
                        <button class="btn-read" onclick="actualitesManager.showActualiteDetails(${
                          actu.id
                        })">
                            Lire la suite
                        </button>
  ...
  filterActualites() {
    ...
    const filteredActus = this.actualites.slice(1).filter((actu) => {
      const matchSearch =
        actu.titre.toLowerCase().includes(searchTerm) ||
        actu.contenu.toLowerCase().includes(searchTerm);
      const matchFilter =
        this.currentFilter === "all" ||
        actu.categorie.toLowerCase() === this.currentFilter;
  ...
  async showActualiteDetails(actualiteId) {
    try {
      ...
      console.error(
        "Erreur lors du chargement des détails de l'actualité:",
        error
      );
    }
  }
  ...
  async loadPopularContent() {
    try {
      ...
      this.popularCities.innerHTML = villes
        .slice(0, 3)
        .map(
          (ville) => `
      ...
      this.popularSites.innerHTML = sites
        .slice(0, 3)
        .map(
          (site) => `
                <div class="popular-item">
